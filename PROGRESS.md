...
**Last updated:** 2026-04-29 (doc consistency audit + DESIGN.md v3 cleanup pass; M3-day-config TODO captured; interview-synthesis open decisions marked resolved)
...
**Overall:** Calendar Day 22 since first commit (2026-04-08), still in pre-W1 homework — W1 has not started. Environment fully set up, both captain interviews complete, 8 interview-re-anchor decisions baked into BUILD.md. Key findings: multi-manager premise confirmed; 주민번호 in club Excel is first-7-only (PIPA general info, not 고유식별정보); Supabase architecture preserved (Drive pivot considered and rejected); members schema grows by 6 fields. Remaining before W1: Play Console verification (waiting on Google), Naver Band write-scope unblocked or fallback path locked.
...
- **2026-04-29** — **BUILD.md fix pass.** 6 small fixes layered on top of the cross-doc audit: (1) Eng Review Decision 17 marked superseded by CEO Decision 23 (M3 PROTECTED, not cut-on-W9-fail), (2) CEO Review log's "None overrides any prior decision" claim corrected to acknowledge the Decision 23 → Decision 17 supersedence, (3) Part 0.7 prefaced with a 2026-04-29 field-update banner reflecting that the happy-path steps 1-5 are stale (2 rejections, escalation in flight) and step 6 is the live path, (4) Part 0.7 step 2 rewritten with three concrete lessons from the rejected submissions: GitHub Pages instead of `.placeholder`/`.local` URLs, public repo required, Korean app name `리베로` mandatory, Service type must be Android (not 미선택), (5) Cut Valves table Naver Band entry rewritten to reflect current escalation status with the three forks (fixable / blocked / no-reply), (6) Recommended Next Steps item 7 rewritten — "End of W9 hard decision point on M3" replaced with the M3-PROTECTED framing where Band response status determines which M3 implementation flavor ships, not whether M3 ships at all.
- **2026-04-29** — **Cross-doc consistency audit + DESIGN.md v3 prose cleanup.** Audited all 9 markdown files (DESIGN/BUILD/PROGRESS/TODOS/CLAUDE/README + 2 interview notes + 2 scratchpad files). DESIGN.md was the heaviest source of staleness — fixed: (a) Revenue Model rewritten to acknowledge auto-Band is now free-tier (CEO Decision 23 dissolved the Pro hook), (b) System Architecture diagram swapped Cloudflare Worker → Supabase Edge Function, (c) Module 4 references renumbered to Module 3 throughout prose (modules section, effort table, schedule slack, success criteria, open questions, next steps), (d) Week-1 Prototype Gate expanded from 2 gates to 5 (Eng Review Decision 13), (e) Offline writes / Room paragraph rewritten to in-memory StateFlow only, (f) Invite flow (end-to-end) rewritten from App Links + assetlinks.json + domain → 6-char codes (Decision Reversal 3), (g) "Buy domain" + "Cloudflare account" deleted from Next Steps, (h) Reviewer Concerns #4 (Module 4 cut) marked resolved by CEO Decision 23, (i) Open Questions resolved entries marked ✅ inline. Also captured a v1.1 TODO for configurable practice days for the Band auto-post poll (weekly default + per-poll holiday override).
- **2026-04-29** — **Sharesheet/Intent fallback strategized for Module 3 if Naver Band API access is permanently denied** (still 2 rejections, escalation in flight). Recommendation captured in conversation: Android `Intent.ACTION_SEND` fires Band's compose screen pre-populated, captain taps once to post. Loses Band's native poll widget (text-with-comments instead) but slots cleanly into the v1.1 attendance-as-M3-extension TODO since comment-parsing then becomes the only RSVP signal. Not yet written into TODOS.md as a formal entry — pending Band escalation reply.
